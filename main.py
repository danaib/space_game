...
try:
    ...
    img_dir = sys._MEIPASS
    sound_folder = sys._MEIPASS
except Exception:
...
pygame.display.set_caption("Kitty Space Shooter")
...
def main_menu():
    ...
    ready.play()
    screen.fill(BLACK)
    draw_text(screen, "Elise, GET READY!", 40, WIDTH/2, HEIGHT/2)
    pygame.display.update()
...
PIXEL_FONT = pygame.font.Font(pygame.font.match_font('arial'), 24)
LARGE_FONT = pygame.font.Font(pygame.font.match_font('arial'), 48)

    # Display game over screen
def game_over_screen(score, high_score):
        screen.fill(BLACK)
        game_over_text = LARGE_FONT.render("GAME OVER", True, WHITE)
        score_text = PIXEL_FONT.render(f"Your Score: {score}", True, WHITE)
        high_score_text = PIXEL_FONT.render(f"High Score: {high_score}", True, WHITE)
        prompt_text = PIXEL_FONT.render("Press Q to Quit and R to Restart", True, WHITE)
...
async def main():
    ...
    background = pygame.image.load(path.join(img_dir, 'option1.JPG')).convert()
    ...
    player_img = pygame.image.load(path.join(img_dir, 'kitty_small.PNG')).convert()
    ...
    while running:
        ...
        screen.blit(background, (0, scroll_y - background_rect.height))  # Upper part
        ...
        scroll -= 2  # Adjust the speed of scrolling here
        scroll = scroll % background.get_height()  # Reset when it reaches the height of the background

        # Draw the scrolling background
        screen.blit(background, (0, -scroll))  # First image (scrolling up)
        screen.blit(background, (0, -scroll + background.get_height())) 
